...
- Во время ожидания AI показывается короткий индикатор `печатаю` с анимированным троеточием, без предположения о типе выполняемого анализа.
...
- Основной интерфейс проверяется не только на большом desktop monitor, но и на типичном 13-inch laptop viewport.
- Assistant не должен требовать горизонтальной прокрутки sidebar.
- При недостатке ширины context panel скрывается, а main chat получает приоритет.
- На laptop viewport плотность header, tabs, quick commands, chat bubbles и input уменьшается без потери функций.
...
- На узких экранах интерфейс может менять layout, но функции из этого контракта не исчезают.

## 9. One-port production mode
...
3. После изменений пройти UI checklist по затронутому разделу на desktop и laptop viewport.
4. Запустить:
