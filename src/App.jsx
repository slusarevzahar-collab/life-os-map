...
function canComplete(node) { return node?.kind === 'task' && Boolean(node.sourceId); }
...
function SideList({ map, routeDepth, snapshot, onOpen, onSelect, onComplete, busyTaskId }) {
  ...
  return <aside className="sideList"><div className="sideListHead"><div><small>{hasPlanetChildren ? 'Содержимое ветки' : 'Задачи ветки'}</small><strong>{map.title}</strong></div><b>{items.length}</b></div>{items.length ? <div className="sideItems">{items.map((item) => { const nested = Boolean(item.children?.length || item.taskList?.length); const taskComplete = canComplete(item); return <div className="sideItemRow" key={item.id}><button className="sideItemMain" onClick={() => nested && !isLeafNode(item) ? onOpen(item.id) : onSelect(item)}><span>{item.icon}</span><div><b>{shortText(item.title, 46)}</b><small>{isLeafNode(item) ? item.status || item.summary : `${item.tasks || 0} задач · открыть ветку`}</small></div></button>{taskComplete ? <button className="doneMini" disabled={busyTaskId === item.sourceId} onClick={(event) => { event.stopPropagation(); onComplete(item); }}>{busyTaskId === item.sourceId ? '…' : 'Done'}</button> : null}</div>; })}</div> : <div className="emptySide"><b>Список пуст</b><p>Backend подключён, но у этой ветки нет связанных задач или они не совпали по Project/Goal. Проверь названия проекта в Notion.</p></div>}<div className="sideMeta"><span>source: {sourceLabel}</span><span>tasks: {connected.tasks ? 'live' : 'no'}</span><span>goals: {connected.goals ? 'live' : 'no'}</span></div></aside>;
}

function DetailCard({ node, onClose, onComplete, busyTaskId }) {
  if (!node) return null;
  const taskComplete = canComplete(node);
  return <motion.aside className="detailCard" initial={{ y: 28, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 28, opacity: 0 }}><button className="closeDetail" onClick={onClose}>×</button><div className="detailHead"><span>{node.icon}</span><div><small>{node.status || node.subtitle}</small><h2>{node.title}</h2></div><Ring value={node.progress} /></div><p>{node.summary || 'Описание пока не заполнено.'}</p>{taskComplete ? <div className="detailActions"><button className="doneButton" disabled={busyTaskId === node.sourceId} onClick={() => onComplete(node)}>{busyTaskId === node.sourceId ? 'Сохраняю…' : 'Пометить выполненной'}</button></div> : null}{node.details?.length ? <div className="detailList">{node.details.slice(0, 4).map((item, index) => <div key={index}><b>{index + 1}.</b>{item}</div>)}</div> : null}</motion.aside>;
}
...
function App() {
  ...
  const loadSnapshot = useCallback(() => {
    ...
    return fetchSnapshot().then((data) => { setSnapshot(data); setApiState(data.meta?.source?.includes('mock') ? 'mock data' : 'connected'); return data; }).catch((error) => { setSnapshot(emptySnapshot('api-offline', error.message)); setApiState('api offline'); throw error; });
  }, []);

  useEffect(() => { let active = true; loadSnapshot().catch(() => {}); return () => { active = false; void active; }; }, [loadSnapshot]);
  ...
  const completeTask = async (node) => {
    if (!canComplete(node)) return;
    setBusyTaskId(node.sourceId);
    setToast('Сохраняю Done в Notion…');
    try {
      await patchTask(node.sourceId, { status: 'Done', progress: 100, nextAction: 'Done: выполнено через Life OS Map.' });
      await loadSnapshot();
      setSelected(null);
      setToast('Готово: задача помечена Done в Notion.');
