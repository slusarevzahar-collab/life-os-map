...
- убраны дублирующие описания роли Assistant, название активной модели и технические подписи маршрутов;
- AI resource UI теперь одна общая шкала `Ресурс AI`, агрегированная по доступному облачному пулу; она не показывает модель, requests/day и tokens/min отдельными строками;
- добавлена история чатов в sidebar: несколько независимых локальных сессий, `+ Новый`, переключение между чатами, автоматический заголовок по первому сообщению и миграция старого `lifemap.assistant.chat.v3:*` storage;
- на mobile/Fold история доступна через кнопку `История` и отдельный drawer, потому что основной sidebar там скрыт.
...
- история пока локальна конкретному браузеру и домену, не синхронизируется через Notion/backend;
- target-сессии сохраняют компактный snapshot контекста, без полного snapshot LifeMap;
- не ломать legacy migration старых локальных чатов.
...
- точные provider/model diagnostics оставлять только для status/debug API, а не для обычного UI.
