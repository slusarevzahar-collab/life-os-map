...
export function buildSignalFromTelegramUpdate(update = {}) {
  ...
  return {
    ...
    telegram: { updateId: update.update_id, messageId: message.message_id, chatId: message.chat?.id, chatType: message.chat?.type, userId: message.from?.id, username: message.from?.username || message.chat?.username || '', source, sourceUrl: linkInfo.sourceUrl, telegramPostUrl: linkInfo.telegramPostUrl, forwardedFrom: linkInfo.forwardedFrom, entityUrls: linkInfo.entityUrls, document: message.document ? { fileId: message.document.file_id, fileUniqueId: message.document.file_unique_id, fileName: message.document.file_name, mimeType: message.document.mime_type, fileSize: message.document.file_size } : null },
  };
}

export async function enrichSignalWithTelegramDocument({ signal, botToken }) {
  const document = signal?.telegram?.document;
  if (!signal || !botToken || !document?.fileId || !textDocumentCandidate({ file_name: document.fileName, mime_type: document.mimeType, file_size: document.fileSize })) return signal;
  try {
